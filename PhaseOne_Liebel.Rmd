...
## Scale touch to team-based
(adjusting for team touch culture)
...
## Social Network Strength: Player Touch Concentration and Role Dynamics
...
We also considered a broader definition of touch involvement using the `PlayersInvolved` field. This includes all players present in the interaction, not just the toucher or touchee. Patterns echoed those seen in the prior plots, with a few players per team often dominating touch events.
...
## Touch Frequency Over the Course of a Match
...
### Histogram: Raw Match Time (Minutes)
```{r Histogram_TouchesTime, fig.width=8, fig.height=4}
...
### Histogram: Touches by % of Match Completion
```{r Histogram_TouchesTime_PercentTime, fig.width=8, fig.height=4}
Histogram_TouchesTime_PercentTime
```
This normalized histogram shows how touches distribute across matches regardless of game length. This framing allows for cleaner interpretation.

### Final Figure: Smoothed Touch Distribution Across Time
```{r histogram_smoothline, fig.width=9, fig.height=5}
